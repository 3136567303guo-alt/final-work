...
## 二、 深度 AI 交互与试错记录 (AI Interaction & Debugging Log)
...
## 三、 结果分析与讨论 (Analysis & Discussion)

通过运行代码（见附录），我得到了两张关键的误差分析图。

### 1. 任务1分析：为什么 Simpson 法是神？

**观察图片**：`task2_fig1.png` (数值积分误差分析)

*   **斜率对比**：在双对数坐标系下，蓝线（梯形法）的斜率明显平缓，参考线显示斜率为 2；而橙线（Simpson 法）极其陡峭，与斜率为 4 的参考线平行。
*   **数据实证**：
    *   当 $n$ 从 4 增加到 8（翻倍）时，梯形法误差缩小约 4 倍 ($2^2$)。
    *   同等条件下，Simpson 法误差缩小了约 16 倍 ($2^4$)！
*   **结论**：Simpson 法利用抛物线拟合，比梯形法的直线拟合更能贴合 $\sin(x)$ 这种光滑曲线。在工程上，为了达到同样的精度，Simpson 法需要的计算量远小于梯形法。

### 2. 任务2分析：无穷的处理

*   **截断法**：当截断到 $[-5, 5]$ 时，误差就已经非常小了。这说明高斯函数 $e^{-x^2}$ 能量高度集中。
*   **换元法**：在修正了 $dx/dt$ 的导数项后，我成功把无穷积分转化为了有限积分。虽然代码中需要处理分母为 0 的边界情况（加了极小量 `epsilon`），但这证明了数学变换在数值计算中的威力。

### 3. 任务3分析：蒙特卡洛的代价

**观察图片**：`task2_fig2.png` (蒙特卡洛收敛分析)

*   **收敛规律**：图中的点主要沿着一条斜率为 $-0.5$ 的参考线分布。
*   **物理意义**：这验证了 $\text{Error} \propto \frac{1}{\sqrt{N}}$ 的规律。
*   **优缺点对比**：相比于 Simpson 法惊人的 $O(h^4)$ 收敛速度，蒙特卡洛法实在是太慢了。要让精度提高 10 倍，必须多算 100 倍的点。但它的优势在于“不挑食”——对于无法解析的高维复杂积分，它是唯一的解法。

---

## 四、 核心代码实现 (Core Implementation)
...
```
## 五、 实验结果展示 (Visual Results)

# 数值积分与误差分析 结果报告

## 1. 任务一：误差收敛阶数验证
![误差收敛阶数验证](task2_fig1.png)
